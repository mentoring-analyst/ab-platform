...
**Бонус-уровень**: если хочешь собрать полноценный продактовый дашборд руками —
в стеке есть Superset, BI-инструмент, в котором продакты смотрят дашборды
в настоящих компаниях. Поднимается отдельной командой:

```bash
cd ~/ab-platform && make up-full
```

Докер скачает образ (~1.5 ГБ при первом запуске), Superset появится на
http://localhost:8089 (admin/admin); первый запуск инициализируется минуту-две —
если страница пустая, подожди и обнови. Гайд со всеми запросами для дашборда:
