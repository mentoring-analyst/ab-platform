...
в админке — до статистического анализа, решения и поэтапной раскатки.
...
|:-------|:---------------|:------|:---------------|
| **Сплитовалка** (FastAPI) | админка экспериментов, сплитование, автоматический расчёт метрик | http://localhost:8000 | — |
...
а часть базы — «спящие» пользователи, которые почти не заходят в приложение,
как и в настоящем проде.
...
Докеру нужно **4 ГБ памяти**. Проверить и поднять:
...
Поднимется весь стек: базы + сплитовалка + генератор (~3 ГБ). При первом старте
...
(бесплатный) или DataGrip (платный, но есть бесплатный план для некомерческого использования).
...
1. Сколько уникальных пользователей активны в день? В неделю? Какая доля базы
   вообще не появляется в данных — и что это значит для набора аудитории
   эксперимента?
...
1. Возьми шаблон [templates/experiment_card.md](templates/experiment_card.md)
   и заведи по нему страницу в Notion. Открой доступ ментору.
2. Заполни все разделы, кроме расчёта выборки (он — следующий шаг).

Ревью идёт в комментариях к карточке — после правок карточка считается
утверждённой. Без утверждённой карточки эксперимент не запускается, как
и в настоящей компании. Все дальнейшие материалы проекта — расчёт выборки,
результат A/A, ноутбук анализа, выводы — прикладывай в эту же карточку.
Вопросы по кейсу и контексту — ментору в телеграм (**@asiriatskiy**).
...
Ноутбуки проекта запускай **локально** (VS Code с расширением Jupyter или
`jupyter notebook`) — Google Colab не подойдёт: он работает на сервере Google
и не видит докер на твоей машине.
...
(раздел «Размер выборки и длительность»).
...
- варианты и доли трафика — из карточки. Доли задаются **от всего трафика
  ...
  а 60% в него не попадают. Меньшая доля — меньше цена ошибки, если вариант
  окажется вредным;
...
## Шаг 6. Мониторинг эксперимента

Здесь ничего поднимать не нужно: расчёт метрик уже работает. Фоновый воркер
платформы раз в 45 секунд реплицирует абшницу в ClickHouse и досчитывает
дневные метрики всех активных экспериментов по закрытым виртуальным дням
(в больших компаниях эту работу делает оркестратор вроде Airflow — здесь
она встроена в сплитовалку).

**Весь мониторинг — на странице эксперимента в [админке сплитовалки](http://localhost:8000)**:

- живой счётчик попавших в тест;
- **SRM-проверка** со светофором: «доли соответствуют плану» / «подозрительно» /
  «SRM — метрикам верить нельзя»;
- **кривая набора аудитории** по группам — по ней видно, когда наберётся
  расчётная выборка из карточки (и на каком плато остановится набор);
- **метрики по дням** — значения появляются по мере закрытия виртуальных дней.
...
Эксперимент работает срок из карточки: 2 виртуальные недели ≈ 2.5 часа реального
времени. Смотреть на страницу эксперимента можно в любой момент, принимать
решение раньше срока — нельзя: промежуточные p-value случайно пересекают порог
значимости, и остановка на первом «прокрасе» ломает заявленную alpha.

Досрочная остановка допустима по двум причинам: загорелся SRM или деградирует
защитная метрика. Рост целевой метрики причиной остановки не является.

---

## Шаг 8. Анализ и решение

Срок вышел — останови эксперимент кнопкой и проведи анализ по ноутбуку
[templates/experiment_analysis.ipynb](templates/experiment_analysis.ipynb):
...
Запусти, дай поработать несколько виртуальных дней и проверь на странице эксперимента,
что эффект воспроизводится на широкой аудитории.

Вечно holdout не держат: контрольная группа недополучает улучшение, и чем
дольше она живёт, тем дороже это обходится продукту.

---

## Итог проекта

В карточке к концу проекта лежит всё:

1. Все разделы заполнены: расчёт выборки, результат A/A, итоги анализа, решение.
2. **Ноутбук анализа** — sanity-checks, тесты, интервалы, выводы.
3. **Скриншот страницы эксперимента** из админки (кривая набора, SRM, метрики).
...
лимиты ClickHouse, занятые порты, полный сброс стенда.
