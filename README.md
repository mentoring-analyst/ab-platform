...
| **Superset** | BI для продактового дашборда (бонус-уровень) | http://localhost:8089 | `admin` / `admin` |
...
Пока эксперимент набирает данные, включи автоматический расчёт метрик.

До сих пор у тебя работал лёгкий профиль стека: базы, сплитовалка, генератор.
Теперь нужно поднять расчётную часть — Airflow. Открой **Терминал**, перейди
в папку проекта и выполни команду:
...
«full»: докер скачает недостающие образы (при первом запуске — пара гигабайт,
несколько минут) и запустит их **вдобавок** к уже работающим контейнерам —
эксперимент при этом не прерывается. Понадобится ~8 ГБ памяти, выделенной
докеру (если `make check` ругался — подними лимит, как описано в шаге 0.1).
Проверь, что всё поднялось: `make ps` — новые контейнеры должны быть в статусе Up.

Больше делать ничего не нужно: Airflow сам, каждые 5 минут, считает дневные
метрики всех активных экспериментов. **Весь мониторинг — на странице
эксперимента в админке**:
...
Заглянуть под капот расчёта можно в Airflow (http://localhost:8081,
admin/admin) — DAG `ab_experiment_metrics`,
[его код](airflow/dags/ab_experiment_metrics.py).

**Бонус-уровень**: вместе с полным профилем поднимается и Superset
(http://localhost:8089, admin/admin) — BI-инструмент, в котором продакты
смотрят дашборды в настоящих компаниях. Если хочешь собрать полноценный
продактовый дашборд своими руками — гайд со всеми запросами:
[docs/superset_dashboard.md](docs/superset_dashboard.md). Для прохождения
проекта это не обязательно.

Первый запуск сервисов инициализируется минуту-две (миграции баз) — если
страница пустая или не открывается, подожди и обнови (Cmd+R / F5).
...
реального времени). Подглядывать на страницу эксперимента можно и нужно — принимать решение
...
3. **Скриншот страницы эксперимента** из админки (кривая набора, SRM, метрики);
   если делал бонусный дашборд в Superset — его скриншот тоже.
