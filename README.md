...
через год. Возьми шаблон [templates/experiment_card.md](templates/experiment_card.md),
скопируй в `experiments/pricing_point_estimate/card.md` в своём форке и заполни
все разделы, кроме расчёта выборки (он — следующий шаг).

**Сдача — через Pull Request**: форкни репозиторий, закоммить карточку, открой PR
и позови ментора в ревью. Без утверждённой карточки эксперимент не запускается —
как и в настоящей компании.

---

## Шаг 3. Размер выборки и длительность
...
проверка симуляциями (Monte Carlo) → сверка с реальным размером аудитории
(`audience/preview` в админке посчитает его по твоим фильтрам).
...
(раздел «Размер выборки и длительность») и обнови PR.

---

## Шаг 4. A/A-тест: проверь инструмент перед боем

В бигтехе перед важным экспериментом сплитовалку проверяют A/A-тестом: две
одинаковые группы, никакого различия между ними. Если «эффект» нашёлся — сломан
сплит или логирование, и запускать боевой тест нельзя.
...
Это добавит Airflow (http://localhost:8081, admin/admin) и Superset
...
1. **PR с карточкой эксперимента** — все разделы, включая расчёт выборки,
   результат A/A, итоги анализа и решение.
